...
// The planner imports only TypeScript types. Transpile its source to an isolated
// ESM data URL so Node can exercise the production logic without a browser.
const source = readFileSync(new URL('../src/lib/dailyPlan.ts', import.meta.url), 'utf8')
const javascript = ts.transpileModule(source, {
  compilerOptions: { module: ts.ModuleKind.ESNext, target: ts.ScriptTarget.ES2022 },
}).outputText
const { buildDailyPlan } = await import('data:text/javascript;base64,' + Buffer.from(javascript).toString('base64'))

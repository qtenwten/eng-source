...
function Reading({onSave,onActivity}:{onSave:Props['onAddInbox'];onActivity:Props['onRecordActivity']}){
  ...
  return <><div className="tool-switch">{readingTexts.map((item,i)=><button key={item.id} className={i===index?'active':''} onClick={()=>{setIndex(i);setDone(false)}}>{item.cefr} · {item.minutes} мин</button>)}</div><article className="reading-card"><p className="eyebrow">EXTENSIVE READING · {text.cefr}</p><h2>{text.title}</h2><p className="reading-text">{text.text}</p><div className="target-list">{text.targets.map(target=><button key={target[0]} onClick={()=>onSave(target[0],target[1],contextForTarget(text.text,target[0]))}><strong>{target[0]}</strong><span>{target[1]}</span><small>＋ в Inbox</small></button>)}</div><button className="primary-action" onClick={finish} disabled={done}>{done?'Зачтено ✓':'Прочитал ради смысла, без постоянного перевода'}</button></article></>
}

function Listening({onSave,onActivity}:{onSave:Props['onAddInbox'];onActivity:Props['onRecordActivity']}){
  const[index,setIndex]=useState(0),[revealed,setRevealed]=useState(false),[done,setDone]=useState(false),[playCount,setPlayCount]=useState(0);const clip=listeningClips[index]
  const reset=(i:number)=>{setIndex(i);setRevealed(false);setDone(false);setPlayCount(0)}
  const play=()=>{speak(clip.text);setPlayCount(value=>value+1)}
  const finish=()=>{if(playCount>0&&!done){onActivity('listening',2);setDone(true)}}
  return <><div className="tool-switch">{listeningClips.map((item,i)=><button key={item.id} className={i===index?'active':''} onClick={()=>reset(i)}>{i+1}</button>)}</div><article className="listening-card"><p className="eyebrow">AUDIO FIRST · {clip.cefr}</p><h2>Сначала только услышать</h2><p className="tool-lead">Прослушай хотя бы один раз без текста. Transcript — поддержка после собственной попытки понять смысл, а не замена listening.</p><button className="listen-big" onClick={play}>▶ Прослушать {playCount?`· ${playCount}`:''}</button>{!revealed?<button className="secondary-action" disabled={!playCount} onClick={()=>setRevealed(true)}>{playCount?'Показать transcript':'Сначала прослушай'}</button>:<div className="transcript"><strong>{clip.text}</strong><span>{clip.translation}</span>{clip.targets.map(target=><button key={target[0]} onClick={()=>onSave(target[0],target[1],clip.text)}>＋ Сохранить: {target[0]} — {target[1]}</button>)}</div>}<button className="primary-action" onClick={finish} disabled={!playCount||done}>{done?'Listening записан ✓':'Закончить listening'}</button></article></>
}
...
function Errors({learner,onAdd,onPractice}:{learner:LearnerState;onAdd:Props['onAddError'];onPractice:Props['onPracticeError']}){
  ...
  })}{!sorted.length&&<Empty text="Ошибок пока нет. Они будут появляться из writing-задач или их можно добавить вручную."/>}</div></>
}
...
function ActiveVocabulary({learner,allItems}:{learner:LearnerState;allItems:LearningItem[]}){
  ...
  return <><p className="tool-lead">«Active» теперь требует реального output: карточная память может довести элемент до recall, одно употребление — до contextual use, повторное употребление — до active.</p><div className="mastery-legend">{order.map(stage=><span key={stage}>{masteryLabels[stage as keyof typeof masteryLabels]}</span>)}</div><div className="mastery-list">{rows.slice(0,60).map(({item,stage,uses})=><article key={item.id}><div><strong>{item.answer}</strong><small>{item.translation}{uses?` · использовано в output: ${uses}`:''}</small></div><span className={`stage stage-${stage}`}>{masteryLabels[stage]}</span></article>)}</div></>
}

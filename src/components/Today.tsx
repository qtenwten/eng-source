...
export function Today({learner,stats,dueCount,newCount,sessionLimit,availability,onStart,onOpenModule}:Props){
  ...
  const status=dueCount>0?`${dueCount} элементов готовы к повторению`:newCount>0?`Повторения закрыты · ${newCount} новых элементов ждут знакомства`:'Карточки закрыты. Переключись на живой язык — чтение, аудирование или говорение.'
...
function planSubtitle(id:string,fallback:string,dueCount:number,newCount:number,sessionLimit:number,availability:Availability){
  ...
  if(id==='listening')return'аудио → расшифровка после попытки'
  if(id==='speaking')return'говорение / письмо с возвратом'
  return fallback
}

import type { LearnerError,MasteryStage,MemoryState,ProductiveMode } from '../types'
...
export function analyzeProduction(response:string,targets:string[],mode:ProductiveMode):FeedbackResult{
  ...
  const hits=targets.filter(target=>normalized.includes(target.toLowerCase())).length
  ...
  if(matched.length)feedback.push(`Нашёл ${matched.length} повторяемую грамматическую конструкцию для журнала ошибок.`)
  else feedback.push('По локальным правилам явных типовых ошибок не найдено. Это не заменяет полноценную AI-проверку.')
  return{feedback,errors:matched.map(rule=>({incorrect:rule.incorrect,correct:rule.correct,explanation:rule.explanation,source:'writing' as const}))}
}

export function masteryStage(memory?:MemoryState):MasteryStage{
  if(!memory)return'unseen'
  if(memory.reviews<=1)return'recognition'
  if(memory.reviews<=2||memory.stability<2)return'recall'
  if(memory.reviews<=4||memory.stability<7)return'context'
  return memory.lastRating==='good'||memory.lastRating==='easy'?'active':'context'
}
